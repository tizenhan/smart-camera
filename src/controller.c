...
#define GPIO_PIN_MOTION_SENSOR 18
Eina_Bool _timer_cb(void *data)
{
	...
	ret = resource_read_infrared_motion(GPIO_PIN_MOTION_SENSOR, &value);
	retv_if(ret < 0, ECORE_CALLBACK_RENEW);

	_I("Detected motion value : %u, count : %d", value, count);
	...
	if (count > 0) {
		...
		ret = access(ad->latest_image_filename, R_OK);
		if (!ret) { // if exists
			_D("Try to remove file");
			ret = remove(ad->latest_image_filename);
			if (ret < 0) _E("cannot remove file[%s]", ad->latest_image_filename);
		}
	}
...
static bool service_app_create(void *data)
{
	...
	motion_timer = ecore_timer_add(TIMER_DURATION, _timer_cb, ad);
	goto_if(!motion_timer, ERROR);
