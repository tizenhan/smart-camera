...
static void __capturing_cb(camera_image_data_s *image, camera_image_data_s *postview, camera_image_data_s *thumbnail, void *user_data)
{
	...
	/* ANALYZE : This project doesn't use this data */
...
static void __start_capture(void *user_data)
{
	...
	/* ANALYZE : __capturing_cb is just called before saving data in app-side */
	/* ANALYZE : __completed_cb has to be called because you have to call 'camera_start_preview' */
...
/* ANALYZE : THIS IS CALLED IN PREVIEW THREAD */
static void __camera_preview_cb(camera_preview_data_s *frame, void *user_data)
{
	...
	/* ANALYZE : For all calls that need to happen in the main loop (most EFL functions do),
...
int resource_camera_init(preview_image_buffer_created_cb preview_image_buffer_created_cb, void *user_data)
{
	...
	/* ANALYZE : Sets the quality of a still image, which is captured. */
	...
	/* ANALYZE : This project doesn't capture any images. */
	...
	/* ANALYZE : State is CAMERA_STATE_NONE, CAMERA_STATE_CREATED, CAMERA_STATE_PREVIEW, CAMERA_STATE_CAPTURING, CAMERA_STATE_CAPTURED */
	...
	/* ANALYZE : a preview callback to be called once per frame when previewing. */
	...
	/* ANALYZE : Auto Focus is CAMERA_ATTR_AF_NONE, CAMERA_ATTR_AF_NORMAL, CAMERA_ATTR_AF_MACRO, CAMERA_ATTR_AF_FULL */
...
int resource_camera_stop_preview(void)
{
	...
	if (state == CAMERA_STATE_CAPTURING || state == CAMERA_STATE_CAPTURED) {
		_D("Camera is now capturing");
		return -1;
	}
...
/* ANALYZE : This project never use this capturing function. */
int resource_camera_capture(capture_completed_cb capture_completed_cb, void *user_data)
{
	...
	/* ANALYZE : Why does it return void instead of int? */
